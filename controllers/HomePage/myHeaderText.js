const HeaderText = require("../../models/HomePage/headerText");
...
const headerText = async (req, res) => {
  try {
    ...
    const data = await HeaderText.create({
      title,
      subTitle,
    });
...
const getHeaderText = async (req, res) => {
  try {
    const data = await HeaderText.find();
    if (data.length ==0) {
...
const updateHeaderText = async (req, res) => {
  try {
    ...
    const data = await HeaderText.findOne();
