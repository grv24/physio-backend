const SocialMediaLink = require("../../models/footer/socialMediaLinkModel");
const fs = require("fs");

const socialMediaLink = async (req, res) => {
  try {
    ...
    if (!socialMediaName  || !link) {
      return res
        .status(400)
        .json({ message: "Please enter all the required fields!" });
    }

    const data = await SocialMediaLink.create({
      socialMediaName,
      logo: file,
      link,
    });
...
const updateSocialMediaLink = async (req, res) => {
  try {
    ...
    const data = await SocialMediaLink.findById(id);
    if (!data) {
